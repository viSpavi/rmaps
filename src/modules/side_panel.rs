...
impl SidePanel {
    ...
    pub fn draw(&mut self, size: (f32,f32), offset: (f32,f32), helper: &mut WindowHelper, graphics: &mut Graphics2D) {
        ...
        //draw border lines
        graphics.draw_line(self.bounds.top_left(), self.bounds.bottom_left(), BORDER_LINES_WIDTH, BORDER_LINES_COLOR);
        graphics.draw_line(self.bounds.top_right(), self.bounds.bottom_right(), BORDER_LINES_WIDTH, BORDER_LINES_COLOR);
        graphics.draw_line(self.bounds.top_left(), self.bounds.top_right(), BORDER_LINES_WIDTH, BORDER_LINES_COLOR);
        graphics.draw_line(self.bounds.bottom_left(), self.bounds.bottom_right(), BORDER_LINES_WIDTH, BORDER_LINES_COLOR);
        ...
        for module in MODULES.read().unwrap().iter() {
            ...
            let formatted_text = self.font.layout_text(module.read().unwrap().get_name().as_str(), FONT_SIZE, TextOptions::new().with_wrap_to_width(background.width()*(1.0-TEXT_H_PADDING_RATIO), TextAlignment::Left));

            let hitbox = Rectangle::from_tuples(
                (self.bounds.top_left().x+background.width()*TEXT_H_PADDING_RATIO, y),
                (self.bounds.top_left().x+background.width()*TEXT_H_PADDING_RATIO+formatted_text.width(), y+formatted_text.height()
            ));

            graphics.draw_text((hitbox.top_left().x, hitbox.top_left().y), TEXT_COLOR, &formatted_text);
